...
special_words = set([
    'nenies', 'ĉiel', 'cxiel', 'ĉies', 'iel', 'ĉiam', 'kie', 'kiu', 'tiu',
    'iom', 'cxiu', 'nenio', 'tiel', 'cxio', 'kiom', 'nenie', 'cxia', 'tio',
    'ĉiom', 'ial', 'neniu', 'tiam', 'kia', 'cxiom', 'ie', 'ĉia', 'kiam',
    'cxie', 'io', 'kio', 'tia', 'kiel', 'nenial', 'tial', 'ties', 'e', 'tie',
    'neniel', 'ies', 'neniom', 'ĉie', 'nenia', 'iam', 'ĉio', 'kial', 'tiom',
    'an', 'cxial', 'iu', 'kies', 'ia', 'ĉial', 'neniam', 'ĉiu', 'cxies',
    'cxiam', 'desur', 'depost', 'disde'
])

to_root = set([
  "onin","ciis","sxiis","ŝiis","cxuo","ĉuo","oli","liu","cin","mio", "cia","trea","gxio","ĝio",
  "cio","cin","tree","onio"
])

pronouns_and_stuff = set([a + b for a in ["mi", "vi", "li", "sxi", "gxi", "ni", "ili", "ĝi", "ŝi", "si"] for b in ['', 'n', 'a', "an", "j", "aj", "ajn"]])

special_words = special_words | pronouns_and_stuff

make_root = set([("AFFIX","AFFIX","ROOT"),("SPECIAL", "SPECIAL"),
                 ('SPECIAL', 'SPECIAL', 'ENDING'), ('AFFIX', 'ROOT'),
                 ('ROOT', 'AFFIX')])

SPECIAL = set([
    "expression", "pronoun", "expression", "adverb", "article", "conjunction",
    "preposition"
])
  ...
  for i in range(len(tagged_text)):
    ...
    if text in weird:
      ...
      if i != len(tagged_text) - 1 and tagged_text[i + 1][1] == "ENDING":
        s += text
      elif i != len(tagged_text) - 1 and (text + tagged_text[i + 1][0]
                                          in words):
        s += text
      else:
        s += text + " "
...
def wordsegtonew(seg, tags):
  ...
  if "ROOT" in tags and tags.count("ROOT") > 1:
    ...
    seg = TAG_SEP.join(seg[:firstroot] + ["".join(seg[firstroot:lastroot])] +
                   seg[lastroot:])
  ...
  if seg.split(TAG_SEP)[-1] in [
      "i", "ia", "as", "os", "us", "u", "o", "a", "e", "on", "an", "en", "oj",
      "aj", "ojn", "ajn", "aŭ"
  ]:
    tagcopy = list(tags)
    tagcopy[-1] = "ENDING"
